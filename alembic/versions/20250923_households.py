...
def upgrade():
    ...
    if not inspector.has_table('family_households'):
        op.create_table(
            'family_households',
    ...
    # 检查索引是否已存在，如果不存在才创建
    if not inspector.has_index('family_households', 'ix_family_households_slug'):
        op.create_index('ix_family_households_slug', 'family_households', ['slug'], unique=True)

    if not inspector.has_table('family_members'):
        op.create_table(
            'family_members',
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
    ...
    if not inspector.has_index('family_members', 'ix_family_members_household_id'):
        op.create_index('ix_family_members_household_id', 'family_members', ['household_id'])

    if not inspector.has_table('family_member_accounts'):
        op.create_table(
    ...
    if not inspector.has_index('family_member_accounts', 'ix_family_member_accounts_member_id'):
        op.create_index('ix_family_member_accounts_member_id', 'family_member_accounts', ['member_id'])
    
    if not inspector.has_index('family_member_accounts', 'ix_family_member_accounts_user_id'):
